...
static const char *INDEX_HTML =
    ...
    "<style>body{font-family:Arial,Helvetica,sans-serif;background:#0f172a;color:#e2e8f0;padding:16px;display:flex;align-items:flex-start;gap:16px;}"
    "h1{font-size:20px;margin:0 0 12px;}label{display:block;margin:0 0 4px;}"
    "input{width:100%;padding:6px;border-radius:6px;border:1px solid #1e293b;background:#0b1220;color:#e2e8f0;}"
    "button{margin:0;padding:10px 14px;border:0;border-radius:8px;background:#22c55e;color:#0b1220;font-weight:600;cursor:pointer;}"
    ...
    "button:active{transform:scale(0.98);}section{margin-bottom:18px;padding:14px;border:2px solid #38bdf8;border-radius:14px;background:#1e293b;box-sizing:border-box;}"
    "canvas{border:1px solid #1e293b;background:#0b1220;} .status{font-size:14px;margin:12px 0 0;} .left{flex:1;max-width:420px;display:flex;flex-direction:column;justify-content:space-between;min-height:1290px;} .right{flex:1;min-width:0;}"
    ".panel{display:flex;flex-direction:column;justify-content:space-between;height:100%;} .afm-row{display:flex;gap:8px;align-items:center;margin-bottom:10px;} .afm-row label{min-width:130px;white-space:nowrap;} .afm-row input{flex:1;min-width:0;}"
    ".button-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:12px;} .button-grid button{width:100%;} .button-row{display:flex;gap:8px;margin-top:6px;} .button-row button{flex:1;} .checkbox-row{display:flex;align-items:center;gap:8px;margin-top:12px;} .section-header-row{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px;} .radio-group{display:flex;align-items:center;gap:10px;flex-wrap:wrap;} .radio-group label{margin:0;display:flex;align-items:center;gap:4px;cursor:pointer;}"
    "</style></head><body><div class='left'><section class='panel'><div class='section-header-row'><h1>Stepper Motor Control</h1></div>"
    "<label>Max speed (200-10000 pps)</label><input id='vmax' type='number' min='200' max='10000' value='4000'>"
    "<button onclick='setMaxSpeed()'>Set</button>"
    "<label>Motor Enable</label><div class='button-row'><button onclick='setMotorEnable(1)'>Enable</button><button onclick='setMotorEnable(0)'>Disable</button></div>"
    "<label>Move distance (pulses, +/-)</label><input id='distance' type='number' value='1000'>"
    "<button onclick='moveOnce()' class='secondary'>Move</button>"
    "<div class='button-grid'>"
    "<button id='jogPos' onmousedown='jog(1,true)' onmouseup='jog(1,false)' ontouchstart='jog(1,true)' ontouchend='jog(1,false)' class='secondary'>Jog +</button>"
    ...
    "<button onclick='stopNow()' class='danger'>Stop</button>"
    "</div></section><section class='panel'><div class='section-header-row'><h1>AFM Scan Control</h1></div>"
    "<div class='afm-row'><label>X Scan Size (um)</label><input id='x_scan_size' type='number' min='0' max='30' step='0.1' value='10.0'></div>"
    "<div class='afm-row'><label>Y Scan Size (um)</label><input id='y_scan_size' type='number' min='0' max='30' step='0.1' value='10.0'></div>"
    "<div class='afm-row'><label>X Offset (um)</label><input id='x_offset' type='number' min='0' max='30' step='0.1' value='0'></div>"
    "<div class='afm-row'><label>Y Offset (um)</label><input id='y_offset' type='number' min='0' max='30' step='0.1' value='0'></div>"
    "<div class='afm-row'><label>Scan Rate (Hz)</label><input id='scan_rate' type='number' min='0' max='10' step='0.1' value='1.0'></div>"
    "<div class='afm-row'><label>Samples (0-1024)</label><input id='samples' type='number' min='0' max='1024' value='250'></div>"
    "<div class='afm-row'><label>Lines (0-1024)</label><input id='lines' type='number' min='0' max='1024' value='250'></div>"
    "<div class='button-grid'>"
    "<button onclick='updateScanParams()'>Update</button><button id='scanBtn' onclick='toggleScan()' class='secondary'>Scan Start</button>"
    "<button onclick='setDirection(true)'>Upward</button><button onclick='setDirection(false)'>Downward</button>"
    "</div>"
    "<div class='checkbox-row'><label for='saveData'>Save data after scan</label><input id='saveData' type='checkbox'></div>"
    "</section></div><div class='right'><section>"
    "<div class='section-header-row'><h1>Height Image</h1><div class='radio-group'>"
    "<label><input type='radio' id='bitmapTrace' name='bitmapType' value='trace' checked>Trace</label>"
    "<label><input type='radio' id='bitmapRetrace' name='bitmapType' value='retrace'>Retrace</label>"
    "</div></div>"
    "<canvas id='bitmapCanvas' width='1024' height='1024'></canvas></section>"
    "<section><canvas id='curveCanvas' width='1024' height='200'></canvas></section>"
    "<div class='status' id='statusBar'>Scanning: Stopped | Direction: Upward | Current Line: 0</div></div>"
    ...
    "let dataArray = [];"
    "function afmhot(v){v=Math.max(0,Math.min(1,v));let r=0,g=0,b=0; if(v<0.25){const t=v/0.25; r=255*t; g=0; b=0;} else if(v<0.5){const t=(v-0.25)/0.25; r=255; g=160*t; b=0;} else if(v<0.75){const t=(v-0.5)/0.25; r=255; g=160 + 95*t; b=0;} else {const t=(v-0.75)/0.25; r=255; g=255; b=255*t;} return [Math.floor(r), Math.floor(g), Math.floor(b)];}"
